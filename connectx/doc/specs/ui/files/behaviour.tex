...
should be modal, since game can start before it is completed.
  ...
  The section title is a readonly \ttt{Label} displaying the constant string 'Game'.
  ...
  The section title is a readonly \ttt{Label} displaying the constant string
  'Game board'.
...
\begin{enumerate}
  \item \ttt{Label},  readonly : Displays the constant string 'Do you want
                                 to read the manual online?'.
  \item \ttt{Label},  readonly : Displays the constant string 'You will be
                                 redirected to the documentation webpage
                                 where help pages are mainained for every
                                 release.'.
  \item \ttt{Button}, readonly : Displays the constant string 'Cancel'.
                                 Clicking on this button closes the dialog
                                 and no help page is accessed.
  \item \ttt{Button}, readonly : Displays the constant string 'Read online'.
...
The about \ttt{Connect X} dialog is a small dialog dedicated to giving software
related information. It should be minimal and to the point. It is composed of the
following elements:

\begin{enumerate}
  \item \ttt{Image},  readonly : Displays the \ttt{Connect X} logo.
  \item \ttt{Label},  readonly : Displays the constant string 'Connect X $A.A$',
                                 where $A.A$ is the \ttt{Connect X} version number.
  \item \ttt{Label},  readonly : Displays the constant string 'Connect X is a
                                 multi-user scallable Four-in-a-row game.'.
  \item \ttt{Label},  readonly : Displays the constant string 'Copyright © 20AA-20BB
                                 Éric Poirier' where 20AA and 20BB represent the
                                 years for which the copyright is active.
  \item \ttt{Button}, active   : Displays the constant string 'Credits'. When
                                 clicked, this button conjures up the Credits
                                 dialog (see \cref{dlg:Credits} for more
                                 information).
  \item \ttt{Button}, active   : Displays the constant string 'Licence'. When
                                 clicked, this button conjures up the Licence
                                 dialog (see \cref{dlg:Licence} for more
                                 information).
  \item \ttt{Button}, active   : Displays the constant string 'Close'. When clicked,
...
The credits dialog is a simple dialog composed only of a \ttt{Textbox} in which
some text presents the names and email addresses of the people who have
contributed significantly to \ttt{Connect X}. The names/emails should be
...
The licence dialog is only composed of a \ttt{Textbox} containing the official
...
Some actions in the \ttt{Connect X} application might trigger message boxes:
...
\begin{table}[H]
  ...
  \end{longtable}
  \caption{The \ttt{Connect X} message dialogs}
