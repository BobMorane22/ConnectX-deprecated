...
Library uses the namespace \ttt{std} for its members. Furthermore, it is good 
practice to use a \ttt{\#define} to hide the namespace name. This way, if the 
namespace name changes (idealy, it should not), the program can sucessfully 
recompile with almost no work. For example, this is a set of \ttt{\#define}s 
